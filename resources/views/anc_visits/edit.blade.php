        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Update the recorded antenatal care visit details.
                </p>
            </div>

            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('anc-visits.show', $ancVisit) }}"
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl border border-gray-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100 bg-amber-50">
                    <h3 class="text-lg font-semibold text-gray-900">Update ANC Visit Details</h3>
                    <p class="text-sm text-gray-600 mt-1">
                        Modify the visit record and save the updated information.
                    </p>
                </div>

                <form action="{{ route('anc-visits.update', $ancVisit) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="mother_id" id="mother_id"
                                ...
                                @foreach($mothers as $id => $name)
                                    <option value="{{ $id }}"
                                        {{ old('mother_id', $ancVisit->mother_id) == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date"
                                   name="visit_date"
                                   id="visit_date"
                                   value="{{ old('visit_date', \Carbon\Carbon::parse($ancVisit->visit_date)->format('Y-m-d')) }}"
                                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            @error('visit_date')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number"
                                   ...
                                   id="gestation_weeks"
                                   min="1"
                                   value="{{ old('gestation_weeks', $ancVisit->gestation_weeks) }}"
                                   placeholder="e.g. 24"
                                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   ...
                                   name="weight"
                                   id="weight"
                                   value="{{ old('weight', $ancVisit->weight) }}"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="blood_pressure"
                                   id="blood_pressure"
                                   value="{{ old('blood_pressure', $ancVisit->blood_pressure) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes"
                                      ...
                                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('notes', $ancVisit->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100">
                        <button type="submit"
                                class="inline-flex items-center justify-center rounded-lg bg-amber-500 px-6 py-3 text-sm font-semibold text-white shadow hover:bg-amber-600 transition">
                            Update ANC Visit
                        </button>

                        <a href="{{ route('anc-visits.show', $ancVisit) }}"
                           class="inline-flex items-center justify-center rounded-lg bg-blue-50 px-6 py-3 text-sm font-semibold text-blue-700 shadow-sm hover:bg-blue-100 transition">
                            View Record
                        </a>
